...
def _resolve_data_dir():
    candidates = [os.getenv("DATA_DIR"), "/data", "/tmp/data", os.path.join(os.path.dirname(__file__), "data")]
    for p in candidates:
        if not p: continue
        try:
            Path(p).mkdir(parents=True, exist_ok=True)
            t = Path(p) / ".write_test"; t.write_text("ok", encoding="utf-8"); t.unlink(missing_ok=True)
            return p
...
bandit = PolicyArms(); knn = PatternKNN(); bayes = BayesModel(); bans = CFPBan()
detector = DoLDetector(bandit); tm = TradeManager()

application: Application | None = None
PTB_LOOP: asyncio.AbstractEventLoop | None = None

def _is_admin_uid(uid) -> bool:
    if not ADMIN_CHAT_ID or str(ADMIN_CHAT_ID).strip() in ("", "0", "None"): return True
    return str(uid) == str(ADMIN_CHAT_ID)

def _read_trades_safe() -> List[dict]:
    try:
        if not os.path.exists(TRADES_PATH): return []
        with open(TRADES_PATH, "r", encoding="utf-8") as f: data = json.load(f); return data if isinstance(data, list) else []
    except Exception: return []

def _save_trades(rows: List[dict]):
    try:
        with open(TRADES_PATH, "w", encoding="utf-8") as f: json.dump(rows, f, ensure_ascii=False, indent=2)
    except Exception: pass

def _append_trade(row: dict):
    rows = _read_trades_safe(); rows.append(row); _save_trades(rows)

def _now_iso_utc(): return datetime.utcnow().replace(microsecond=0).isoformat() + "Z"
def _p(v, nd=2): 
    try: return f"{float(v):.{nd}f}"
    except Exception: return str(v)

def _is_tls_hiccup(err: Exception) -> bool:
    s = str(err).lower(); return ("decryption_failed" in s) or ("bad record mac" in s)

async def safe_send(context: ContextTypes.DEFAULT_TYPE, **kw):
    for i in range(3):
        try: return await context.bot.send_message(**kw)
        except Exception as e:
            if _is_tls_hiccup(e): await asyncio.sleep(0.6*(i+1)); continue
            raise

async def safe_reply(update: Update, **kw):
    if not update or not update.message: return
    for i in range(3):
        try: return await update.message.reply_text(**kw)
        except Exception as e:
            if _is_tls_hiccup(e): await asyncio.sleep(0.6*(i+1)); continue
            raise
...
def _score_R_from_logs() -> float:
    ...
    base = 0.15*sl8; cluster = 0.4 if sl4>=2 else 0.0
...
def _score_M_from_market() -> float:
    ...
    vol_score = 0.3 if ar<=0 else float(max(0.0, min(1.0, math.exp(-((ar-0.007)**2)/(2*(0.006**2))))))
    body = (df["close"]-df["open"]).abs().astype(float); tr=(df["high"]-df["low"]).astype(float)
...
def _decide_mode(Q: float, M: float, R: float):
    ...
    prev = AI_STATE["mode"]; now=time.time()
...
async def refresh_ai_mode(context: ContextTypes.DEFAULT_TYPE=None, announce: bool=True):
    try:
        ...
        if changed and announce and ADMIN_CHAT_ID and context:
            ...
            if mode=="CAUTIOUS": spec=f"disp +15%, fvg +10%, return −5%, gate +0.07; cooldown={COOLDOWN_CAUTIOUS_MIN}m"
            elif mode=="AGGRESSIVE": spec=f"disp −20%, fvg −18%, return +20%, gate −0.10; cooldown={COOLDOWN_AGGR_MIN}m"
            else: spec=f"baseline: disp −20%, fvg −15%, return +15%, gate −0.09; cooldown={COOLDOWN_BASELINE_MIN}m"
            await safe_send(context, chat_id=ADMIN_CHAT_ID, text=f"🧠 AI Mode: {mode} (A={A:.2f}) — {spec}", parse_mode=ParseMode.MARKDOWN)
    except Exception: pass

def _current_tuning() -> Dict[str,float]:
    m=AI_STATE["mode"]
    if m=="CAUTIOUS":   return {"disp_mult":1.15,"fvg_mult":1.10,"return_mult":0.95,"conf_adj":+0.07}
    if m=="AGGRESSIVE": return {"disp_mult":0.80,"fvg_mult":0.82,"return_mult":1.20,"conf_adj":-0.10}
...
def _enforce_levels_from_dollars(st: TradeState, risk_usd: float, tp1_pts: float, tp2_pts: float, qty_btc: float):
    ...
    risk_dist=_dist_from_pnl_usd(risk_usd, qty_btc); tp1_dist=float(tp1_pts); tp2_dist=float(tp2_pts)
    if st.side=="BUY":
        stop_px=entry-risk_dist; tp1_px=entry+tp1_dist; tp2_px=entry+tp2_dist
...
async def scan_once(context: ContextTypes.DEFAULT_TYPE):
    ...
    if tm.active and not tm.active.closed: return
    df5 = await asyncio.to_thread(get_klines, "5", 800, False)
    if df5 is None or len(df5)<210: return
    tuning=_current_tuning()
    try: sig = detector.find(df5, bayes, knn, bandit, bans, tuning=tuning)
    except TypeError: sig = detector.find(df5, bayes, knn, bandit, bans)
    if not sig:
        df15 = await asyncio.to_thread(get_klines, "15", 600, False)
        if df15 is not None and len(df15)>=210:
            try: sig = detector.find(df15, bayes, knn, bandit, bans, tuning=tuning)
            except TypeError: sig = detector.find(df15, bayes, knn, bandit, bans)
    if not sig or not tm.can_open(): return
    ...
    await asyncio.to_thread(_append_trade, {
        "time": _now_iso_utc(), "symbol": SYMBOL, "side": st.side,
        "entry": float(st.entry_ref), "sl": float(st.stop_px),
        "tp1": float(st.tp1_px), "tp2": float(st.tp2_px),
        "result": "OPEN", "tf": "5m", "source": "live"
    })
    if ADMIN_CHAT_ID:
        setup_lines=_full_setup_text(sig)
        txt=(f"🎯 <b>ENTRY LIVE</b> — <code>{st.side}</code> on <code>{SYMBOL}</code>\n"
...
async def manage_once(context: ContextTypes.DEFAULT_TYPE):
    if not (tm.active and not tm.active.closed): return
    df1 = await asyncio.to_thread(get_klines, "1", 120, True)
    df5 = await asyncio.to_thread(get_klines, "5", 210, True)
    if df1 is None or df5 is None or len(df1)<30 or len(df5)<50: return
    st = tm.active
    ai_conf = 0.78; tm.maybe_stretch(st, ai_conf)
    ev = tm.manage(st, df1, df5)
    if not ev:
        if not ASSISTANT_MODE or not ADMIN_CHAT_ID: return
        try:
            cur=float(df1["close"].iloc[-1]); mae=abs(cur-(st.entry_price or st.entry_ref))
            delta=abs(st.entry_ref - st.stop_px) or 1.0; mae_ratio=float(min(1.0, mae/delta))
        ...
        return
    if ADMIN_CHAT_ID:
        await safe_send(context, chat_id=ADMIN_CHAT_ID, text=_commentary_trade_event(st, ev["event"], ev["price"]), parse_mode=ParseMode.MARKDOWN)
    if ev["event"] in ("TP2","SL","EARLY_TP2","TP1","EARLY_TP1","CANCELLED"):
        if ev["event"] in ("TP2","SL","EARLY_TP2"): tm.close(st, ev["event"], ev["price"])
        R = trade_reward(st, ev["event"]); bandit.update(st.arm_id, norm_reward(R), weight=1.5 if "TP2" in ev["event"] else 1.0)
        await asyncio.to_thread(_append_trade, {
            "time": _now_iso_utc(), "symbol": SYMBOL, "side": st.side,
            "entry": float(st.entry_price or st.entry_ref), "exit": float(ev["price"]),
            "sl": float(st.stop_px), "tp1": float(st.tp1_px), "tp2": float(st.tp2_px),
            "result": ev["event"], "tf": "5m", "source": "live"
        })
        if ev["event"]=="SL":
            await refresh_ai_mode(context, announce=False)
            mode=AI_STATE["mode"]
            cd_min = COOLDOWN_CAUTIOUS_MIN if mode=="CAUTIOUS" else (COOLDOWN_AGGR_MIN if mode=="AGGRESSIVE" else COOLDOWN_BASELINE_MIN)
            now_ms=int(time.time()*1000); tm.cooldown_until_ms=max(tm.cooldown_until_ms, now_ms + cd_min*60*1000)
            if ADMIN_CHAT_ID:
                await safe_send(context, chat_id=ADMIN_CHAT_ID, text=f"⏳ Cooldown set to {cd_min} min (AI mode: {mode}; chop filters still ON).")
...
async def session_commentary(context: ContextTypes.DEFAULT_TYPE):
    ...
    if 12<=h<=17:
        if _last_session_sent.get("LONDON")!=day and ADMIN_CHAT_ID:
            await safe_send(context, chat_id=ADMIN_CHAT_ID, text="📈 London in play: likely liquidity engineering. Watch for sweep → displacement. Sniper mode ON 🔥")
            _last_session_sent["LONDON"]=day
    if 19<=h<=23:
        if _last_session_sent.get("NY")!=day and ADMIN_CHAT_ID:
            await safe_send(context, chat_id=ADMIN_CHAT_ID, text="⚡️ NY session: breakout probability elevated. Track displacement legs and FVG returns 🚀")
            _last_session_sent["NY"]=day

async def daily_recap(context: ContextTypes.DEFAULT_TYPE):
    if not ADMIN_CHAT_ID: return
    today=datetime.now().strftime("%Y-%m-%d")
    await safe_send(context, chat_id=ADMIN_CHAT_ID, text=(f"📅 <b>Daily Recap — {today}</b>\nAsia: context logged • London/NY: commentary posted\nTrades: check logs.\n🔥 One clean kill > many messy shots."), parse_mode=ParseMode.HTML)

async def weekly_recap(context: ContextTypes.DEFAULT_TYPE):
    if not ADMIN_CHAT_ID: return
    await safe_send(context, chat_id=ADMIN_CHAT_ID, text="📆 <b>Weekly Recap</b>\nWins/Losses, EV improvements, chop avoidance, and session chains summarised. Keep sniping ⚔️", parse_mode=ParseMode.HTML)
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_reply(update, text=f"Online. Symbol `{SYMBOL}`. 1-trade lock; risk ${RISK_DOLLARS:.0f}.", parse_mode=ParseMode.MARKDOWN)
...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await _require_admin(update): return
    try:
        await scan_once(context); await safe_reply(update, text="Scan done.")
    except Exception as e:
...
async def cmd_check_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if df5 is None or df5.empty:
            await safe_reply(update, text="❌ No data from MEXC."); return
        ts = df5.index[-1]
...
async def cmd_backtest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not results:
            await safe_reply(update, text="✅ Backtest complete. No valid setups in the window."); return
...
async def cmd_logs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not trades:
        await safe_reply(update, text="No saved trades yet."); return
    lines=["🧾 Last trades:"]
    for t in trades[-10:]:
        when=t.get("time","?"); sym=t.get("symbol", SYMBOL); side=t.get("side","?")
        ent=t.get("entry","?"); res=t.get("result", t.get("exit_reason","?"))
        src=t.get("source","live"); tf=t.get("tf",""); tf_s=f" [{tf}]" if tf else ""
...
async def cmd_btdebug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        msg=(f"🔎 Backtest Debug ({days}d, {tf}m)\n"
             f"Bars: {d.get('bars',0)}\n"
...
async def cmd_diag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        df1 = await asyncio.to_thread(get_klines, "1", 120, True)
        parts=[]
        if df5 is None or df5.empty: parts.append("5m: ❌ no data")
        else:
            ts5=df5.index[-1]; ts5 = ts5.tz_localize("UTC") if ts5.tz is None else ts5.tz_convert("UTC")
            parts.append(f"5m: {len(df5)} bars (last {ts5.isoformat()})")
        if df1 is None or df1.empty: parts.append("1m: ❌ no data")
        else:
            ts1=df1.index[-1]; ts1 = ts1.tz_localize("UTC") if ts1.tz is None else ts1.tz_convert("UTC")
...
async def cmd_force_unlock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if tm.active and tm.active.closed: tm.active=None
...
async def cmd_aistats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        s=AI_STATE
        msg=(f"🤖 AI Stats\nMode: {s['mode']} (A={s['A']:.2f}), hold ≥{AI_MODE_HOLD_MIN}m (≈{mins_left}m left)\n"
             f"Q={s['Q']:.2f} • M={s['M']:.2f} • R={s['R']:.2f}\n"
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if ADMIN_CHAT_ID: await safe_send(context, chat_id=ADMIN_CHAT_ID, text=msg)
    except Exception: pass
...
async def _ptb_runner():
    ...
    host=os.environ.get("RENDER_EXTERNAL_HOSTNAME"); tok=os.getenv("TOKEN") or TOKEN
    if host and tok:
        try: await application.bot.delete_webhook()
        except Exception: pass
        try: await application.bot.set_webhook(url=f"https://{host}/{tok}")
        except Exception: pass
    await application.start()
    await asyncio.Event().wait()

def _start_background_ptb():
    def _run(): asyncio.run(_ptb_runner())
...
def telegram_webhook():
    try:
        ...
        if application is None: return "init", 200
        update = Update.de_json(data, application.bot)
        if PTB_LOOP is None: return "loop-missing", 200
...
@app.get("/health")
def health(): return f"OK PTB=v20+ symbol={SYMBOL}", 200
